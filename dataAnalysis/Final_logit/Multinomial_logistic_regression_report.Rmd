...
Given the fact that the target feature was a percentage we tried to run a logistic regression to predict when the room was more likely to be empty, 25\%, half, 75\% and 100\% occupied. 

Below we describes step by step all the analyses performed.
...
```  
Then we made a query to the database, in order to get all the ground truth data collected in room B.002, B.004 and B.006 from 9:00 a.m. to 17 a.m. and the correspondent Wi-Fi Log measured in that time frame and rooms. 
...
Initially we set all the categorical variables as factors and then we printed the descriptive statistic for all the features.
```{r}
...
Furthermore for the variables Wifi_Average_logs and Wifi_Max_logs, it seems that there are few outliers, since the median is lower than the mean and the max values are far higher than the mean values. We will going to explore this issues with histogram and boxplots.
...
```
<br><br><br>Form the histograms we could see that the distribution of the feature Wifi Maximum_client (i.e. the Maximum number of devices logged in one hour lecture) was skewed to the left, indicating that the in the majority of the lecture were counted no more than 40 people. Furthermore, we could see that there are potential outliers (values > 150). Similar pattern was observed for the feature Wifi_Average_logs.
...
box1 <- ggplot(AnalysisTable, aes(x = factor(0), y = Wifi_Average_logs)) + geom_boxplot() + theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

#box plot for the maximum number of clients variable
box2 <- ggplot(AnalysisTable, aes(x = factor(0), y =Wifi_Max_logs)) + geom_boxplot() + theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) 

#box plot for the Time continuous variable
box3 <- ggplot(AnalysisTable, aes(x = factor(0), y = Time)) + geom_boxplot() + xlab("Time") + theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  
...
```
All the ranges of occupancy were equally represented in all the three rooms, indicating that room was not affecting the occupancy.
...
* *Binned occupancy ~ Average Wifi occupancy*;

* *Binned occupancy ~ Average Wifi occupancy + Room*;

* *Binned occupancy ~ Average Wifi occupancy + Time*;

* *Binned occupancy ~ Average Wifi occupancy + Course_Level*;

* *Binned occupancy ~ Average Wifi occupancy + Room + Time*;
* *Binned occupancy ~ Average Wifi occupancy + Room + Course_Level*;
* *Binned occupancy ~ Average Wifi occupancy + Time + Course_Level*;
* *Binned occupancy ~ Average Wifi occupancy + Room + Time + Course_Level*;
...
The 10 fold cross validation was carried out manually using the package *carret*, as showed in the following tutorial: (http://amunategui.github.io/multinomial-neuralnetworks-walkthrough). 
For each model we estracted the overall accuracy and the AIC and we picked as best model the model with the lowest MSE and AIC. We are aware that for a better comparison among models is better to use AICc, but the package used only provided AIC estimates.

# Results

###CASE1: Wifi average logs
...
| Models                                                                 | Accuracy           |AIC
|------------------------------------------------------------------------|--------------------|----
|Binned_occupancy ~ 1                                                    |`r mean(totalAccuracy.null)`|`r mean(totalAIC.null)`
|Binned_occupancy ~ Wifi_Average_logs                                    |`r mean(totalAccuracy.avg)` |`r mean(totalAIC.avg)`
|Binned_occupancy ~ Wifi_Average_logs + Room                             |`r mean(totalAccuracy.avg.room)`|`r mean(totalAIC.avg.room)`
|Binned_occupancy ~ Wifi_Average_logs + Factor_Time                      |`r mean(totalAccuracy.avg.time)`|`r mean(totalAIC.avg.time)`
|Binned_occupancy ~ Wifi_Average_logs + Course_Level                     |`r mean(totalAccuracy.avg.time)`|`r mean(totalAIC.avg.time)`
|Binned_occupancy ~ Wifi_Average_logs + Room + Factor_Time               |`r mean(totalAccuracy.avg.room.time)`|`r mean(totalAIC.avg.room.time)`
|Binned_occupancy ~ Wifi_Average_logs + Room + Course_Level              |`r mean(totalAccuracy.avg.room.level)`|`r mean(totalAIC.avg.room.level)`
|Binned_occupancy ~ Wifi_Average_logs + Factor_Time + Course_Level       |`r mean(totalAccuracy.avg.time.level)`|`r mean(totalAIC.avg.time.level)`
|Binned_occupancy ~ Wifi_Average_logs + Room + Factor_Time + Course_Level|`r mean(totalAccuracy.avg.full)`|`r mean(totalAIC.avg.full)`


The best model in term of accuracy and AIC was Binned_occupancy ~ Wifi_Average_logs + Room, with an AIC of ~ `r mean(totalAIC.avg.room)` and an accuracy of `r mean(totalAccuracy.avg.room)`.  To assess the importance of the other models in respect to the best one, we can calculate the difference between their AIC value and the best model's AIC. For semplicity, we can say that if the difference between the first best model and the rest is > 2 we can be sure that the model selected was the best one. In this case, the difference between the AIC of the best model and the AIC of the rest of the model was higher than 2. Therfore we are confident with our choice.

###CASE2: Wifi maximum logs
...
|---------------------------------------------------------------------|-------------------------|---
  |Binned_occupancy ~ 1                                                 |`r mean(totalAccuracy.null)`|`r mean(totalAIC.null)`
|Binned_occupancy ~ Wifi_Max_logs                                     |`r mean(totalAccuracy.max)` |`r mean(totalAIC.max)`
|Binned_occupancy ~ Wifi_Max_logs + Room                              |`r mean(totalAccuracy.max.room)`|`r mean(totalAIC.max.room)`
|Binned_occupancy ~ Wifi_Max_logs + Factor_Time                       |`r mean(totalAccuracy.max.time)`|`r mean(totalAIC.max.time)`
|Binned_occupancy ~ Wifi_Max_logs + Course_Level                      |`r mean(totalAccuracy.max.time)`|`r mean(totalAIC.max.time)`
|Binned_occupancy ~ Wifi_Max_logs + Room + Factor_Time                |`r mean(totalAccuracy.max.room.time)`|`r mean(totalAIC.max.room.time)`
|Binned_occupancy ~ Wifi_Max_logs + Room + Course_Level               |`r mean(totalAccuracy.max.room.level)`|`r mean(totalAIC.max.room.level)`
|Binned_occupancy ~ Wifi_Max_logs + Factor_Time + Course_Level        |`r mean(totalAccuracy.max.time.level)`|`r mean(totalAIC.max.time.level)`
|Binned_occupancy ~ Wifi_Max_logs + Room + Factor_Time + Course_Level |`r mean(totalAccuracy.max.full)`|`r mean(totalAIC.max.full)`


The best model in term of accuracy and AIC was Binned_occupancy ~ Wifi_Max_logs + Room with an AIC of  `r mean(totalAIC.max.room)`and an accuracy of `r mean(totalAccuracy.max.room)`. Again the difference between the AIC of the best model and the AIC of the rest of the model was higher than 2. Therfore we are confident with our choice.
However the accuracy and the AIC of the Binned_occupancy ~ Wifi_Average_logs + Room  model was better and we keep it as best model and we will run it on the whole dataset.

final.avg.room <- multinom(Binned_Occupancy ~ Wifi_Average_logs+Room, data=AnalysisTable,maxit=1000)

With the hope of increasing the accuracy of our final model we decided to take into account in the analyses the order that there is among the level of the target feature. To do so, we performed the ordinal regression.
...
| Models                                                                 |Accuracy           |AIC                       
|------------------------------------------------------------------------|-------------------|-----
  |Binned_occupancy ~ 1                                                    |`r mean(totalAccuracy.ord.null)`|`r mean(totalAIC.ord.null)`
|Binned_occupancy ~ Wifi_Average_logs                                    |`r mean(totalAccuracy.ord.avg)` |`r mean(totalAIC.ord.avg)`
|Binned_occupancy ~ Wifi_Average_logs + Room                             |`r mean(totalAccuracy.ord.avg.room)`|`r mean(totalAIC.ord.avg.room)`
|Binned_occupancy ~ Wifi_Average_logs + Factor_Time                      |`r mean(totalAccuracy.ord.avg.time)`|`r mean(totalAIC.ord.avg.time)`
|Binned_occupancy ~ Wifi_Average_logs + Course_Level                     |`r mean(totalAccuracy.ord.avg.time)`|`r mean(totalAIC.ord.avg.time)`
|Binned_occupancy ~ Wifi_Average_logs + Room + Factor_Time               |`r mean(totalAccuracy.ord.avg.room.time)`|`r mean(totalAIC.ord.avg.room.time)`
|Binned_occupancy ~ Wifi_Average_logs + Room + Course_Level              |`r mean(totalAccuracy.ord.avg.room.level)`|`r mean(totalAIC.ord.avg.room.level)`
|Binned_occupancy ~ Wifi_Average_logs + Factor_Time + Course_Level       |`r mean(totalAccuracy.ord.avg.time.level)`|`r mean(totalAIC.ord.avg.time.level)`
|Binned_occupancy ~ Wifi_Average_logs + Room + Factor_Time + Course_Level|`r mean(totalAccuracy.ord.avg.full)`|`r mean(totalAIC.ord.avg.full)`

Binned_occupancy ~ Wifi_Average_logs + Room  was the best model in term of AIC (`r mean(totalAIC.ord.avg.room)`) and accuracy (`r mean(totalAccuracy.ord.avg.room)`). However, the ordinal regression did not improve the accuracy of the multinomial logistic regression. Similar pattern were observed for Wifi_Max_logs as independent variable.

##CASE1: Wifi Maximum logs
```{r, echo=FALSE,results='hide', message=FALSE, warning=FALSE, fig.keep='none'}
#max model
...
|---------------------------------------------------------------------|-------------------|-----
|Binned_occupancy ~ 1                                                 |`r mean(totalAccuracy.ord.null)`|`r mean(totalAIC.ord.null)`
|Binned_occupancy ~ Wifi_Max_logs                                     |`r mean(totalAccuracy.ord.max)` |`r mean(totalAIC.ord.max)`
|Binned_occupancy ~ Wifi_Max_logs + Room                              |`r mean(totalAccuracy.ord.max.room)`|`r mean(totalAIC.ord.max.room)`
|Binned_occupancy ~ Wifi_Max_logs + Factor_Time                       |`r mean(totalAccuracy.ord.max.time)`|`r mean(totalAIC.ord.max.time)`
|Binned_occupancy ~ Wifi_Max_logs + Course_Level                      |`r mean(totalAccuracy.ord.max.time)`|`r mean(totalAIC.ord.max.time)`
|Binned_occupancy ~ Wifi_Max_logs + Room + Factor_Time                |`r mean(totalAccuracy.ord.max.room.time)`|`r mean(totalAIC.ord.max.room.time)`
|Binned_occupancy ~ Wifi_Max_logs + Room + Course_Level               |`r mean(totalAccuracy.ord.max.room.level)`|`r mean(totalAIC.ord.max.room.level)`
|Binned_occupancy ~ Wifi_Max_logs + Factor_Time + Course_Level        |`r mean(totalAccuracy.ord.max.time.level)`|`r mean(totalAIC.ord.max.time.level)`
|Binned_occupancy ~ Wifi_Max_logs + Room + Factor_Time + Course_Level |`r mean(totalAccuracy.ord.max.full)`|`r mean(totalAIC.ord.max.full)`


The best model in term of AIC (`r mean(totalAIC.ord.max.room)`) and accuracy (`r mean(totalAccuracy.ord.max.room)`) was Binned_Occupancy ~ Wifi_Max_logs+Room. The occuracy of this model was not better than the accuracy of the multinomial model with Wifi_Average_logs as descriptive variable. 

Therefore, we are going to considered as best model: Binned_Occupancy ~ Wifi_Average_logs+Room run with a multinomial logistic regression, suggesting that the occupancy of the room was accurately predicted by the average of the Wi-Fi logs measured across the hour and by the room.