...
For running the linear regression we performed k fold cross validation with the aim of testing if the selected model can be applied to new datasets. K fold validation was preferred over the validation set approach and the Leave Out Cross Validation (LOOCV), because it is more robust and more accurate in estimating the test error. The Validation set approach tends to give an over estimate of the test error and the test error is dependent on the observations included randomly in the test set. Furthermore, the LOOCV tends to provide a test error with a high variance, because the folds used to calculating it are correlated among each other. 
In particular in this analysis we are going to perform a 10 fold cross validation, which is pretty standard.

## Results

We run the first model, $Survey occupancy \~ Average Wifi occupancy + Room + Time + Course_Level + Time * Course_Level$, and it did not perform well
<br>
```{r, echo= TRUE}
...
As we can see from the results the overall mean square error was 435. Smaller is the mean square error, higher is the precision of our estimate. Therefore, this model does not fit the data.
Similar results were found when running the same model, but with Wifi_Average counted clients.
...
The overall mean square was 411, suggesting that the model was giving accurate predictions.

For this reasons, we tried to fit a generalised linear model with a Poisson distribution, which is usually used for count data. Count data, in fact, they do not assume negative value and they do not take all the possible values of the real number. Consequently they follow a Poisson distribution.
Since the model suffer of overdispersion we corrected the standard errors using a quasi-GLM model and perform a 10 fold cross validation and we got the following results.
```{r, echo=FALSE, warning=FALSE}
...
k.occupancy$delta
```
The raw cross validation estimate of prediction error was 868 and the adjusted cross validation estimate was 826, which it is pretty high, suggesting that also this model was not accurately predicting the occupancy of the room.
Similar results were found for the average counted client. 
...
k.occupancy2$delta
```
The raw cross validation estimate of prediction error was 734 and the adjusted cross validation estimate 706.

To improve the analysis we tried to remove the outliers, even though it should not make much of a difference given the distribution of the target feature. 
From the hist we could see that max_client and average_client has 2 bin that they seems to be outliers (150 and 200). I remove from the dataset all the rows where max_client is higher than 120.
```{r, echo=FALSE, warning=FALSE}
...
```
Only 3 observations were removed, so only 3 observations were dropped. Looking histograms improved the distribution of average client and maximum client, but not of the counted clients (see below).
```{r, echo=FALSE, warning=FALSE}
...
```
At this point we re-run all the analyses.
```{r, echo=TRUE, warning=FALSE}
...
```
The overall mean square of the linear model with the wifi max ount was 393, which is still pretty high.
We run the model to the whole dataset to see the residuals and we could see that the graphs plotting the residual againist the fitted values display strange pattern. The normality was accettable, while the variance was not homogeneus.
```{r, echo=TRUE}
...
```
Similar results were found for the second model with the average counted clients as response variable. 
```{r, echo=TRUE}
...
````
 The overall mean square was 370 and the model was still pretty bad.

Also the generalise linear model with poisson distribution did not give satisfying results.
```{r,echo=FALSE, warning=FALSE}
