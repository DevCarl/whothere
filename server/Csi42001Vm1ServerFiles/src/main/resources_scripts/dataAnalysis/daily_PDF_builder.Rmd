...
count_plot <-ggplot(data=occupancy_table, aes(x=factor(Time), y=Expected_people, group=1)) + geom_ribbon(data=occupancy_table,aes(ymin=Min_people, ymax=Max_people),alpha=0.2, fill="orangered3") + geom_line(color="orangered2")+ theme_bw() + geom_point(color="orangered2") 
multiplot(occupancy_plot, count_plot, cols=2)+labs(x="Time",y="Predicted number of people in the room")
```
